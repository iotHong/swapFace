            ...
            for i, t in enumerate(timesteps):
                ...
                if isinstance(self.scheduler, LCMScheduler):
                    ...
                    mutual_latents, pred_x0_mutual = ddim_out.prev_sample, ddim_out.pred_original_sample
                        ...
                        # ===== 新增保存逻辑 =====
                def save_latent_as_image(latent, prefix):
                    """将潜在空间张量解码为图片并保存"""
                    with torch.no_grad():
                        # 解码潜在空间
                        output_dir = "./generation_process2" 
                        ...
                        for b in range(image.shape[0]):
                            img = Image.fromarray((image[b] * 255).astype(np.uint8))
                            img.save(os.path.join(output_dir, f"{prefix}_step{i:03d}_batch{b}.png"))

                # 保存 mutual_latents（当前潜在空间）
                save_latent_as_image(mutual_latents, "mutual_latents")

                # 保存 pred_x0_mutual（预测的干净潜在空间）
                save_latent_as_image(pred_x0_mutual, "pred_x0_mutual")
